...
from app.agents import pipeline, classifier, reconciler
...
def test_run_pipeline_stage_error_propagates_and_marks_document_error(db_session, monkeypatch):
    # offline stubs identical to the happy-path test, except the reconciler blows up
    async def fake_classify(text, keys):
        return ("invoice", 0.99)
    monkeypatch.setattr(classifier, "classify_document", fake_classify)
    monkeypatch.setattr(services, "ai_extract", lambda text, fields, path: [
