...
export default function Home(){
 ...
 <div className="mb-6 grid grid-cols-2 gap-3">{[["Documents",processed,FileSearch],["Automation",rateLabel,Activity],["Avg. time",avgTime,Clock3],["Reviews",reviewed,CheckCircle2],["Fields accepted as-is",agreement,ShieldCheck],["Auto-approved",stats?.auto_approved??0,Bot],["Auto-reopen rate",autoReopenRate,TriangleAlert],["Webhook failures",stats?.webhook_failed??0,TriangleAlert]].map(([a,b,Icon]:any)=><div key={a} className="card p-4"><Icon size={17} className="mb-3 text-slate-500"/><p className="label">{a}</p><p className="mt-1 text-2xl font-semibold">{b}</p></div>)}</div>
 ...
 <div className="flex flex-wrap items-center gap-2 border-b border-slate-100 px-5 py-3"><input aria-label="Search documents" placeholder="Search filename…" value={q} onChange={e=>{setOffset(0);setQ(e.target.value)}} className="flex-1 rounded-lg border border-slate-200 px-3 py-1.5 text-sm"/><select aria-label="Filter by status" value={statusFilter} onChange={e=>{setOffset(0);setStatusFilter(e.target.value)}} className="rounded-lg border border-slate-200 px-3 py-1.5 text-sm">{STATUS_OPTIONS.map(s=><option key={s} value={s}>{s===""?"All statuses":s.replaceAll("_"," ")}</option>)}</select><label className="flex items-center gap-1.5 text-sm text-slate-600"><input type="checkbox" checked={autoOnly} onChange={e=>setAutoOnly(e.target.checked)}/> Auto-approved only</label></div>
 <div>{filteredDocs.map(d=><div className="flex items-center justify-between border-b border-slate-100 last:border-0" key={d.id}><button onClick={()=>openDocument(d.id)} className="flex flex-1 items-center justify-between px-5 py-4 text-left hover:bg-slate-50"><div><p className="font-medium">{d.filename}</p><p className="mt-1 text-xs text-slate-500">{d.document_type.replaceAll("_"," ")} · {new Date(d.upload_date).toLocaleDateString()}</p></div><span className="flex items-center gap-1.5"><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${badges[d.status]||badges.uploaded}`}>{d.status.replaceAll("_"," ")}</span>{d.auto_approved&&<span title="Auto-approved" className="rounded-full bg-violet-50 px-1.5 py-0.5 text-[10px] font-semibold text-violet-700">⚡ auto</span>}</span></button>{d.status==="error"&&<button onClick={()=>retry(d.id)} disabled={retrying.has(d.id)} className="mr-5 shrink-0 rounded-lg border border-slate-200 px-2 py-1 text-xs font-semibold hover:bg-slate-50 disabled:opacity-60">{retrying.has(d.id)?"…":"Retry"}</button>}</div>)}{!filteredDocs.length&&<p className="px-5 py-8 text-sm text-slate-500">{q||statusFilter||autoOnly?"No documents match your filters.":"No documents yet. Start with an upload."}</p>}</div>
...
function FieldBody({f,editable,piiSet,hidePii}:{f:any,editable:boolean,piiSet:Set<string>,hidePii:boolean}){let rows:any[]|null=null;try{const p=JSON.parse(f.field_value);if(Array.isArray(p)){const objs=p.filter((r:any)=>r&&typeof r==="object");if(objs.length)rows=objs}}catch{}
 if(rows){const cols=Object.keys(rows[0]);return <div className="mt-2 overflow-x-auto rounded border border-slate-200 bg-white"><table className="w-full border-collapse text-xs"><thead><tr>{cols.map(c=><th key={c} className="border-b border-slate-200 px-2 py-1.5 text-left font-semibold uppercase tracking-wide text-slate-400">{c.replaceAll("_"," ")}</th>)}</tr></thead><tbody>{rows.map((r,i)=><tr key={i}>{cols.map(c=><td key={c} className="border-b border-slate-100 px-2 py-1.5 tabular-nums">{maskValue(String(r[c]??""),piiSet.has(f.field_name),hidePii)||"—"}</td>)}</tr>)}</tbody></table></div>}
 if(!editable)return <p className="mt-2 w-full rounded border border-slate-200 bg-white px-2 py-1.5 text-sm">{maskValue(f.field_value||"",piiSet.has(f.field_name),hidePii)||"—"}</p>;
...
function Review({role,document,schema,onSaved,onRejected,onSavedPending,onExport,onReopened}:{role:Role|null,document:any,schema?:Schema,onSaved:()=>void,onRejected:()=>void,onSavedPending:()=>void,onExport:()=>void,onReopened:()=>void}){const [saving,setSaving]=useState(false);const [reason,setReason]=useState("");const [error,setError]=useState("");const [showAudit,setShowAudit]=useState(false);const [confirmReject,setConfirmReject]=useState(false);const [confirmReopen,setConfirmReopen]=useState(false);
 const [imageUrl,setImageUrl]=useState<string>("");
 const [activeField,setActiveField]=useState<string|null>(null);
 const [hidePii,setHidePii]=useState(true);
 const piiSet=new Set((schema?.fields as any[]||[]).filter((x:any)=>x.pii).map((x:any)=>x.name));
 ...
 if(!document)return <section className="card h-fit p-6"><p className="label">Human review</p><h3 className="mt-1 text-lg font-semibold">Select a processed document</h3><p className="mt-2 text-sm leading-6 text-slate-600">Low-confidence values are highlighted for an operator to verify, correct, and approve.</p></section>; const exportCsv=async()=>{onExport();try{await downloadFile(`/export/${document.id}?format=csv`,`document-${document.id}.csv`)}catch{setError("Export failed. Please retry.")}};return <section className="card h-fit overflow-hidden"><div className="border-b border-slate-100 p-5"><div className="flex items-center justify-between"><p className="label">Human review · {document.document_type.replaceAll("_"," ")}</p><label className="flex items-center gap-1.5 text-xs"><input type="checkbox" checked={hidePii} onChange={e=>setHidePii(e.target.checked)}/> Hide PII</label></div><h3 className="mt-1 font-semibold">{document.filename}</h3><p className="mt-1 text-sm text-slate-500">Confidence {Math.round((document.confidence||0)*100)}%</p>{document.anomalies?.length>0&&<div className="mt-2 flex flex-wrap gap-1.5">{document.anomalies.map((a:any,i:number)=><span key={i} className="flex items-center gap-1 rounded-full bg-amber-50 px-2.5 py-1 text-xs font-semibold text-amber-700"><TriangleAlert size={12}/>{renderAnomaly(a)}</span>)}</div>}</div><div className="grid grid-cols-1 lg:grid-cols-2 gap-3"><DocumentViewer imageUrl={imageUrl} fields={document.fields} activeField={activeField} onPickField={setActiveField}/><div className="max-h-[420px] overflow-auto p-3">{document.fields.map((f:any)=><label key={f.id} onMouseEnter={()=>setActiveField(f.field_name)} className={`mb-2 block rounded-lg p-3 ${f.confidence<.8?"bg-red-50":f.confidence<.9?"bg-amber-50":"bg-slate-50"}${activeField===f.field_name?" ring-2 ring-emerald-400":""}`}><span className="flex items-center justify-between text-xs font-semibold uppercase tracking-wide text-slate-500"><span>{f.field_name.replaceAll("_"," ")}</span><span className="flex items-center gap-2">{f.grounded&&<span className={`rounded px-1.5 py-0.5 text-[10px] font-semibold normal-case tracking-normal ${f.grounded==="grounded"?"bg-emerald-100 text-emerald-700":f.grounded==="ungrounded"?"bg-rose-100 text-rose-700":f.grounded==="unverified"?"bg-amber-100 text-amber-700":"bg-slate-200 text-slate-500"}`}>{f.grounded}</span>}<span>{Math.round(f.confidence*100)}%</span></span></span><FieldBody f={f} editable={editable} piiSet={piiSet} hidePii={hidePii}/>{f.source_quote&&<p className="mt-1 truncate text-[11px] text-slate-400" title={f.source_quote}>from: “{f.source_quote}”</p>}</label>)}</div></div>{editable&&<div className="space-y-3 border-t border-slate-100 p-4"><textarea value={reason} onChange={e=>setReason(e.target.value)} rows={2} placeholder="Optional reason (for reject or save)…" className="w-full rounded border border-slate-200 px-2 py-1.5 text-sm"/>{error&&<p className="text-sm font-medium text-rose-600">{error}</p>}<div className="flex gap-2"><button className="btn-primary flex-1" onClick={()=>submit("approve",onSaved)} disabled={saving}>{saving?"Saving…":"Approve"}</button><button className="btn-secondary" onClick={()=>submit("save",onSavedPending)} disabled={saving}>Save pending</button><button className="rounded-lg bg-rose-600 px-3 py-2 text-sm font-semibold text-white hover:bg-rose-700 disabled:opacity-60" onClick={()=>setConfirmReject(true)} disabled={saving}>Reject</button></div><button className="btn-secondary flex w-full items-center justify-center" onClick={exportCsv}>Export CSV <ArrowUpRight size={14} className="ml-1"/></button></div>}{!!role&&canReview(role)&&isTerminal&&<div className="space-y-3 border-t border-slate-100 p-4">
