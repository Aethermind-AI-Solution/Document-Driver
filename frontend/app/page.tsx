...
export default function Home(){
 ...
 const refresh=async()=>{try{setSchemas(await api("/schemas"));setDocs(await api("/documents"))}catch(e:any){if(e?.status===401){window.location.href="/login"}else{setMessage("API unavailable — start the FastAPI service to connect the workspace.")}}};
 useEffect(()=>{me().then((u:any)=>{setRole(u.role);refresh()}).catch((e:any)=>{if(e?.status===401){window.location.href="/login"}else{setMessage("API unavailable — start the FastAPI service to connect the workspace.")}})},[]);
 ...
 <div className="mt-6 card overflow-hidden"><div className="flex items-center justify-between border-b border-slate-100 px-5 py-4"><div><p className="label">Processing queue</p><h3 className="font-semibold">Recent documents</h3></div><button onClick={refresh} className="text-sm font-semibold text-slate-700">Refresh</button></div><div>{docs.slice(0,6).map(d=><div onClick={()=>openDocument(d.id)} className="flex w-full items-center justify-between border-b border-slate-100 px-5 py-4 text-left last:border-0 hover:bg-slate-50 cursor-pointer" key={d.id}><div><p className="font-medium">{d.filename}</p><p className="mt-1 text-xs text-slate-500">{d.document_type.replaceAll("_"," ")} · {new Date(d.upload_date).toLocaleDateString()}</p></div><span className="flex items-center gap-2"><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${badges[d.status]||badges.uploaded}`}>{d.status.replaceAll("_"," ")}</span>{d.status==="error"&&<button onClick={(e)=>{e.stopPropagation();retry(d.id)}} className="rounded-lg border border-slate-200 px-2 py-1 text-xs font-semibold hover:bg-slate-50">Retry</button>}</span></div>)}{!docs.length&&<p className="px-5 py-8 text-sm text-slate-500">No documents yet. Start with an upload.</p>}</div></div></section>
