...
> **A1** ✅ Shipped (2026-08-15, `feat/p0-gui-sweep`) — frontend queue search/filter/pagination (fixes the 6-doc display cap) + audit-trail render + confirm dialogs → **A2** ✅ Shipped (2026-08-16, `feat/b11-reopen-rework`) — B11 reopen/rework: explicit status state machine (`TRANSITIONS`/`can_transition`), reopen action (approved = admin-only + reason; fields preserved), `/process` blocked on terminal/reopened, `revision` counter in the webhook payload, status-gated Review panel → **A3** ✅ Shipped (2026-08-16, `feat/a3-confidence-eval`) — `document.confidence` = min over required fields (a wrong required field can't be averaged away); offline eval harness `scripts/eval.py` (per-field accuracy, confidence-bucket reliability, grounded-but-wrong rate; correction-derived = honest **upper bound**); `/documents/stats.field_agreement_rate` + "Fields accepted as-is" tile; golden-set loader ships (curating the data is a **B9 prerequisite**) → **A4** ✅ Shipped (2026-08-21, `feat/a4-b9-auto-approve`) — B9 auto-approve/STP: pipeline auto-approves eligible docs behind belt-and-suspenders guards (global `AUTO_APPROVE_ENABLED` off by default + per-type opt-in + confidence floor >0.9 + validator-clean/no-anomalies), stamped as a machine approval (actor=None, `auto_approved`), pushed via `deliver_webhook` (retry + `webhook_status`); admin screen is eval-gated (won't enable a type without enough golden-set data); B11 reopen is the undo → **A5** ✅ Shipped (2026-08-21, `feat/a5-observability`) — B15 observability (lite): structured JSON logging (`LOG_FORMAT`, correlation fields) + admin-only `GET /admin/metrics` (status counts, recent errors, stuck-in-processing, per-stage p50/p95 latency from `pipeline_trace`) + an admin **System Health** screen with a red/amber/green flag banner. **A-track complete.** The full **B15** (external metrics/alerting stack — Prometheus/Grafana, email/Slack alerting, time-series history) remains a later `M`. Next candidates: the enterprise track (`org_id` tenant isolation, retention/B13) or Phase 3 S4/S5.
