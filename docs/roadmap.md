...
> **A1** frontend queue (search/filter/pagination — fixes the 6-doc display cap) → **A2** B11 reopen/rework (+ status state machine) → **A3** confidence calibration + evaluation harness (the keystone — the real "lite B4") → **A4** B9 auto-approve/STP (gated: per-type opt-in default-off, calibrated floor, machine-vs-human audit, kill switch) → **A5** B15 observability (lite).
