...
> **A1** ✅ Shipped (2026-08-15, `feat/p0-gui-sweep`) — frontend queue search/filter/pagination (fixes the 6-doc display cap) + audit-trail render + confirm dialogs → **A2** (next) B11 reopen/rework (+ status state machine) → **A3** confidence calibration + evaluation harness (the keystone — the real "lite B4") → **A4** B9 auto-approve/STP (gated: per-type opt-in default-off, calibrated floor, machine-vs-human audit, kill switch) → **A5** B15 observability (lite).
